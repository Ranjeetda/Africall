...
class Verifyotpprovider with ChangeNotifier {
  ...
  Future<Map<String, dynamic>> sendVerifyRequest(String ccode, String phone, String otp,String email ,String firstname,String lastname,String uipass) async {
    try {
      _isLoading = true;
      notifyListeners();
      String key = '84a5b8c1&d*F8#97';

      String encryptedCcode = encryptString(ccode, key);
      String encryptedPhone = encryptString(phone, key);
      String encryptedOtp = encryptString(otp, key);
      ...
      String encryptedPass = encryptString(uipass, key);

      String mCode = asHex(utf8.encode(encryptedCcode));
      String mPhone = asHex(utf8.encode(encryptedPhone));
      String motp = asHex(utf8.encode(encryptedOtp));
      ...
      String mpass = asHex(utf8.encode(encryptedPass));

      var response = await http.post(
        Uri.parse("http://my.africallconnect.com/munya_api/billing_auto_register/otp_secure_msg.php"),
        headers: {
